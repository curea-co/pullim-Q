...
### 5.x 실제 머지 vs 계획 대응표 (2026-05-15 회고 정정)

| 계획 | 실제 머지 | 머지일 | 비고 |
|---|---|---|---|
| PR-A (Hero 그라디언트 소프트화) | **PR #34** (`b6fb9e0`) | 2026-05-14 | 7개 hero + **추가로 `exam-result` / `infinity onboarding` warn → warn-bg 도 같이 swept** — 본래 PR-C 범위가 PR-A 에 흡수됨 (commit message 마지막 항목 명시) |
| PR-B (환경 텍스트 톤다운) | **PR #36** (`f9a5c56`) | 2026-05-14 | 계획대로 |
| PR-C (Warn 그라디언트 → warn-bg) | **잔여 → PR #44 (2026-05-15)** | 2026-05-15 | 본 plan 신설 시점 2개 파일은 PR-A 에 흡수, 잔여는 `infinity/explain/page.tsx` 시그니처 카드 `from-pullim-warn/15 to /5` 1군데 — 본 PR 에서 sweep |
| PR-D (Shadow 토큰 도입) | **PR #35** (`090c389`) | 2026-05-14 | 계획대로 |
| PR-E (온보딩 CTA 단일화) | **PR #39** (`2e834c8`) | 2026-05-14 | **범위 치환** — 계획의 "온보딩 CTA 단일화" 가 아닌 **`/q/infinity` 시그니처 ring 데코 제거** 로 머지됨. 온보딩 CTA 단일화는 미수행 → §6.3 결정과 함께 후속 plan 으로 분리 검토 |

**핵심 불일치 2건**:
1. **PR-C 범위 흡수**: PR-A 가 warn 그라디언트까지 swept — 계획 분할 의도와 다르지만 산출물 측면에선 동일 효과. 잔여 1군데(`infinity/explain` 시그니처 카드)만 본 PR 에서 처리.
2. **PR-E 범위 치환**: "온보딩 CTA 단일화" → "시그니처 ring 데코 제거" 로 의제 자체가 바뀜. 온보딩 CTA 단일화는 §6.3 권장안에 따라 별도 plan 후속 (현재 PR-A 의 hero 톤다운으로 자연 도드라짐 효과는 일부 충족됨).
