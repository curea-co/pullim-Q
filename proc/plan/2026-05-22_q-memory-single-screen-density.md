...
- [x] 1단계 — Front 카드 헤더에 진행 표시(전체 N개 중 K번째) 추가 — 2026-05-26 G4 단답 5일차 룰 C 발동, PageHeader.description 끝에 `· K/N번째` 부착 (큐 외 진입 시 fallback)
...
- 2026-05-26 — G4 단답 5일차 누적 → CONVENTION §6.E 룰 C 발동 패턴(PR #75·#77 선례) 적용, 1단계만 단독 PR로 잠정 락인. `dueItems(items)` 큐 정렬을 K/N 계산 기준으로 채택(§2 가정 그대로). 큐 외 URL 직접 진입(`queueIndex === -1`) 시 위치 정보 자체 미노출 fallback. 2·3단계는 1단계 모바일 360 회귀 + G4 회신 후 재합의.
- 2026-05-26 (PR #94 머지 후) — G4 단답 채널 회신 도착, 룰 C 발동 1단계 착수에 **retroactive 승인 (Y)**. 잠정 락인 → 정식 승인 승급. 2단계 진입 시 G4 첫 단답 패턴 다시 적용 (룰 C 재발동 불요).
