...
│       │   │   ├── me/{controller, use-cases, service, interface, infrastructure}/  # /api/me read endpoint — 9건 중 하나이므로 동일 계층(§1 완료기준·Phase δ)
...
- 파일럿 코드: **(파일명은 `apps/q/AGENTS.md` §네이밍의 `kebab-case.tsx` 규칙을 따른다 — §0 상 plan 은 AGENTS 하위이므로 파일명 규칙은 AGENTS 가 우선. 아래 `<페이지명>` 은 kebab-case. *컴포넌트 식별자(export 심볼)* 는 React 관례대로 PascalCase 유지 — 본 문서의 `→ FooContainer` 표기는 심볼명이지 파일명이 아니다.)**
  - `apps/q/components/features/q-<도메인>/containers/<페이지명>-container.tsx` 신규 (예: `solve-container.tsx`, export `SolveContainer`) (Container 순수성: 마크업 0줄, 원시값 props 만 전달)
  - `apps/q/components/features/q-<도메인>/presenters/<페이지명>-presenter.tsx` 신규 (예: `review-hub-presenter.tsx`, export `ReviewHubPresenter`)
  - 원본 `components/<도메인>/*.tsx` → `features/q-<도메인>/components/*` git mv (5~7 파일, 기존 kebab-case 파일명 유지)
...
- `q/analysis/page.tsx` (17줄, thin) → **얇은 페이지라도 컨테이너화 생략 없음**(`talk/page.tsx` 와 동일 원칙·§5 트리·Phase 1 규칙 243-245행과 정합): 컴포넌트는 `features/q-analysis/components/*` 로 이동하고, `page.tsx` 는 `AnalysisIntroContainer`(logic 0 이면 Presenter 에 빈 props 만 전달) + `<Suspense>` 래핑만 — feature 컴포넌트 직접 import 금지(page.tsx 는 Container 또는 ServerWrapper 만 import)
...
- **`packages/auth` 최소 구현 (β 로 당김)** — `MockAuthGuard` 가 공용 auth 추상화(`IAuthProvider`) 위에 설계되므로, 현재 placeholder(`packages/auth/src/index.ts` = "Phase β 에서 구현 추가 예정")를 β 에서 **최소 구현**한다: `IAuthProvider` 인터페이스 + 서버용 `MockAuthProvider`(`X-User-Id` 해석) + `packages/auth` 의 `lint`/`test` 스크립트. (β 에서 안 하면 guard 가 임시 타입/로컬 구현을 따로 만들어야 하므로 — η 는 이 위에 client-side `MockAuthProvider` 만 추가)
- `MockAuthGuard` (`X-User-Id` 헤더 → fallback `student_001`) — 위 `packages/auth` 의 `IAuthProvider`/`MockAuthProvider` 추상화 위에 구성
- `RolesGuard` 차용하되 비활성
...
- 완료 기준:
  - **별도 빈 검증용 DB 에서 동등성 확인** (레거시 drizzle 스키마가 살아 있는 기존 dev DB 에 그대로 `migration:run` 하면 기존 테이블/인덱스 충돌로 마이그레이션이 깨지므로): 임시 빈 DB(예: docker-compose 의 별도 `q_verify` DB 또는 `createdb q_typeorm_verify`)를 띄우고 → 그 빈 DB 에 `bun --cwd apps/backend run migration:run` → **TypeORM 적용 결과**의 `pg_dump --schema-only` 와 **기존 drizzle DB**(또는 `apps/q/drizzle/0000_easy_exodus.sql` 적용본)의 `pg_dump --schema-only` 를 diff → **diff 0** (테이블·컬럼·타입·인덱스·제약 동등). 기존 dev DB 의 drizzle 스키마 리셋/제거는 이 시점에 하지 않으며(γ~η 레거시 경로 유지 — 아래 항목), 동등성은 어디까지나 *격리된 빈 DB* 에서만 판정한다
...
- `apps/backend/src/modules/q/`:
  - `me/` — `GET /api/me` (`MeController` + `GetMeUseCase` + repository/interface/infrastructure 계층 — 9 read endpoint 공통 구조와 동일. users 테이블 read 이므로 예외 아님)
  - `infinity/` —
...
- `packages/api-client/src/` 본격 구현:
  ...
  - `client.ts` (fetch wrapper + `X-User-Id` 헤더 자동 주입). **`X-User-Id` 출처 계약(서버/클라이언트 이원화)을 명시**한다 — 서버 컨텍스트(ServerWrapper prefetch)에서는 `headers()` 로 읽지만, **브라우저(Client Component mutation)에서는 `headers()` 가 없으므로 별도 출처가 필요**하다. η 단계 계약: ServerWrapper 가 prefetch 시 읽은 현재 사용자 ID 를 **Container 에 `initialData` 와 함께 `userId` prop(또는 client-side `MockAuthProvider`(`packages/auth`) 컨텍스트값)으로 주입**하고, 브라우저 mutation 시 `client.ts` 는 그 주입된 값을 헤더에 넣는다(`headers()` 호출 없음). 즉 `client.ts` 는 **호출 컨텍스트별로 userId 를 인자/컨텍스트로 받아** 주입하며, 브라우저에서 출처 없이 `student_001` 같은 fallback 으로 잘못 기록되거나 인증 실패로 가지 않도록 **userId 미주입 시 throw** 한다(silent fallback 금지). 이 계약(ServerWrapper → Container `userId` 주입 → client mutation 헤더)을 η 작업 항목·완료 기준에 포함
- `packages/types/src/` 본격 구현 (BE entity DTO 와 1:1 type)
- `packages/auth/src/` — **β 에서 이미 최소 구현된 `IAuthProvider`/서버 `MockAuthProvider`** 위에, η 에서는 **client-side `MockAuthProvider`(브라우저 컨텍스트 userId 제공 — client.ts `X-User-Id` 출처 계약과 짝)** 추가. (auth 추상화 자체는 β 산출물이므로 η 는 클라이언트 측 보강만)
- **`packages/{types,api-client}` 에 `lint`/`test` 스크립트 추가** (본격 구현과 동시), **`packages/auth` 의 `lint`/`test` 는 β 에서 이미 추가됨** — §1 완료 기준의 5 워크스페이스 lint/test 게이트가 이 packages 를 포함하므로, 스크립트 부재 상태로 두지 않는다
