...
- `apps/q/lib/mock/` 의 13 시그니처 전체가 BE surface (entity/use-case/controller 직접 매핑 또는 BE 상수·aggregate view 로 흡수) 로 옮겨져 Q-FE 의 mock 직접 import 0건이 종착점, FE 는 `@pullim-q/api-client` 만 사용 (시그니처별 정확한 처리 방식은 §6 Phase γ 매핑표에 명시)
...
- 검증 명령 (루트):
  ...
  - `bun run lint --filter=@pullim-q/q --filter=@pullim-q/backend` 와 `bun run test --filter=@pullim-q/q --filter=@pullim-q/backend` — `apps/{q,backend}` 만 통과 (현재 `packages/{types,api-client,auth}` 는 lint/test 스크립트 부재. Phase β/η 에서 packages 에 `lint`/`test` 스크립트 추가 시 전체로 확장 — 본 plan 완료 시점 범위는 2개)
...
- 완료 기준:
  ...
  - 30분 bun + NestJS spike 결과 PR 본문에 명시. 실패 시 **사용자 합의 게이트 (G-PM)** 후에만 fallback 진입 — §2.3 의 `bun 유지 + workspace, pnpm 분기 안 함` 결정과 충돌하므로 자동 분기 금지. fallback 옵션은 (1) backend 만 node + pnpm 분기 (planner 정본 결정 그대로) 또는 (2) NestJS 버전 다운, 두 안 중 PM 선택
...
- 완료 기준:
  ...
  - `apps/q/drizzle/`, `apps/q/drizzle.config.ts`, `apps/q/package.json` 의 `db:*` 스크립트, `apps/q/scripts/seed.ts` 를 **이 PR 한 묶음**에서 모두 제거 (§5 폐기 대상과 일관 — 분할 시 중간 머지 깨짐)
...
| **bun + NestJS 호환성** — planner 정본 결정과 동일하나 Q backend 는 아직 spike 미진행 | Phase β 진입 자체 막힐 가능성 | Phase β 진입 직후 30분 spike (`bun --watch apps/backend/src/main.ts` + `nest start --watch`). 실패 시 **G-PM 합의 게이트** 후 fallback (backend 만 node + pnpm 분기 / NestJS 버전 다운) — §2.3 `pnpm 분기 안 함` 결정과 충돌하므로 사용자 재합의 필수 |
