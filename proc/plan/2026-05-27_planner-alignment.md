...
- `rg '@/lib/mock' apps/q --glob '!**/node_modules/**' --glob '!apps/q/lib/mock/**'` 결과 0건 — **`apps/q` 전체**(components·app 뿐 아니라 `lib/**`·`features/*/lib`·`features/*/server` 등 런타임 경로 전부)에서 `@/lib/mock` 직접 import 0건. (single/double quote·`import`·`export ... from`·서브패스 import 전부 포함하도록 따옴표·`from` 한정 없이 패키지 경로 전체를 검사. `lib/mock/` 디렉터리 자신은 제외 — η 마지막 PR 에서 통째 삭제되므로)
...
- 완료 기준:
  - `rg '@/lib/mock' apps/q --glob '!**/node_modules/**' --glob '!apps/q/lib/mock/**'` 0건 — `apps/q` 전체 런타임 경로(`lib/`·`features/*/server` 등 포함, §1 완료 기준과 동일) (single/double quote·`export ... from`·서브패스 import 전부 포함)
  - `rg "drizzle-orm" apps/q` 0건 (서브패스 포함) — `apps/q/package.json` 의존성도 이 PR 내에서 제거되어야 통과 (라인 218)
...
3. **drizzle freeze 룰 명문화 시점** — **지금 즉시(본 plan 합의 시점)** vs Phase β 진입 시점 (Phase α 는 이미 D-Lite 로 완료됐으므로 "α 머지 직후" 는 소급일 뿐 — enforce 가능한 앞 시점만 선택지로 남김. freeze 종료는 라인 470 대로 Phase η 완료로 고정 — 레거시 `app/api`+`lib/db` 제거 시점)
