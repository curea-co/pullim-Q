...
- `lib/db/` Drizzle 즉시 폐기 vs 단계적 entity 동등성 검증 후 폐기 — 후자 우선, **제거는 Phase η**(레거시 `app/api`+`lib/db`+drizzle 자산 일괄 제거 시점, 라인 216·217). γ 에서는 TypeORM entity 를 추가만 하고 drizzle 는 동결·유지. 본 plan 의 시점 표기는 정렬 목표일 뿐.
...
- 검증 명령 (루트):
  ...
  - `bun run lint` 와 `bun run test` — **5 워크스페이스 전부** (`@pullim-q/{q,backend,types,api-client,auth}`) 통과. Phase η 가 `packages/{types,api-client,auth}` 를 placeholder → **본격 구현**으로 전환하므로, **Phase η 작업 항목에 해당 packages 의 `lint`/`test` 스크립트 추가를 포함**하고 완료 기준도 5 워크스페이스 lint/test 통과로 고정한다. (packages 가 본 plan 산출물인 이상 검증 범위에서 제외하면 깨진 패키지 코드로도 완료 판정될 수 있으므로 제외하지 않는다.)
...
| **Container/Presenter** (`features/<domain>/{containers,presenters,components,hooks,lib}/` — `lib/` 는 React 훅이 아닌 순수 유틸/도출 로직 전용, 옵션) | Phase 1~3 완료 (planner-reports, planner-manage, planner-home, planner-onboarding) | **0건** — `components/{infinity,coach,...}/` 평면 구조, 페이지가 mock·router·state 직접 보유 | **대형 갭** — Phase 1~5 전체 적용 필요 |
...
- 완료 기준:
  ...
  - **drizzle 자산(`apps/q/drizzle/`, `drizzle.config.ts`, `db:*` 스크립트, `apps/q/scripts/seed.ts`)은 이 Phase γ 에서 제거하지 않는다** — γ~η 사이 FE 레거시 Drizzle 경로(`app/api/*` + `lib/db`, 라인 217·404)가 살아 있어 회귀검증·regenerate 수단이 필요하므로, 이들은 `app/api`·`lib/db` 와 함께 **Phase η 에서 일괄 제거**(§5 폐기 대상 라인 216·217 과 일관). γ 머지 이후 레거시 drizzle schema 는 **동결**(drizzle freeze, 라인 470)
...
- `apps/q/` 모든 mock import 지점 (`@/lib/mock/...`) 을 `@pullim-q/api-client` 호출로 치환:
  - **`page.tsx` 는 무변** — `apps/q/AGENTS.md` 규칙(`app/*/page.tsx` = Container import + `<Suspense>` 래핑만, 로직 금지)을 그대로 지킨다. `headers()`·server-side prefetch 같은 server-only 로직은 **`page.tsx` 가 아니라 별도 server-only 로더 모듈**(예: `features/q-*/lib/<name>.server.ts` 또는 route segment 의 server util)에 두고, `<Suspense>` 경계 안의 Server Component(Container 가 import 하는 server child)나 Container 의 RSC 부모에서 호출한다. (page-level prefetch 를 page.tsx 에 직접 넣고 싶다면 그건 `apps/q/AGENTS.md`/`CLAUDE.md` 의 page 규칙 변경이 **선행 조건** — 본 plan 임의로 page 책임을 키우지 않는다.)
  - **Container (Client Component, `'use client'`)** — Phase 1~5 정의 그대로: state + router + searchParams + 이벤트 핸들러. 초기 데이터는 **server-only 로더(위 항목)가 RSC 경계에서 prefetch 해 props 로 주입**한 값을 사용 + client-side mutation 시 `@pullim-q/api-client` 호출
...
- 완료 기준:
  - `rg '@/lib/mock' apps/q/components apps/q/app` 0건 (single/double quote·`export ... from`·서브패스 import 전부 포함)
...
| **drizzle 잔존 기간 (Phase α 머지 후 ~ Phase η 완료)** — 레거시 `app/api/*`+`lib/db` 가 η 까지 살아 있어, 그 사이 새 Drizzle route/API 가 추가되면 dual-ORM surface 가 증식 | drizzle 새 호출 추가 시 η 제거 범위 흔들림 | **"drizzle freeze" 기간 = Phase α 머지 직후 ~ Phase η 완료**(레거시 `app/api`+`lib/db` 제거 시점)로 잡는다. 이 기간 내내 (1) 새 drizzle route/API 추가 금지, (2) γ 머지 이후 레거시 drizzle schema 변경 금지 — 두 룰을 각 PR 본문에 명시. (freeze 를 γ 에서 끝내면 δ·ε·ζ 동안 dual-ORM 이 다시 늘 수 있으므로 η 완료까지 유지) |
