...
│   │   │   ├── ui/                          # shadcn 17개
│   │   │   └── brand/                       # 1개
│   │   ├── lib/{tokens,utils.ts,review,store,motion}/   # lib/api 는 제외 — η 에서 @pullim-q/api-client 로 대체·제거 (라인 37 'FE 데이터는 api-client 만')
...
- 완료 기준:
  ...
  - `apps/q/app/api/` · **`apps/q/lib/db/`** · `apps/q/lib/api/` · `apps/q/lib/mock/` · `apps/q/drizzle/` 디렉터리 **전부 부재** (`test ! -d apps/q/lib/db` 등 — 일부 파일만 남는 부분 삭제 불가, 디렉터리 부재가 목표 라인 36)
