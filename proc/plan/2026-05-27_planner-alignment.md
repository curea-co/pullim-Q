...
- `apps/q/` 모든 mock import 지점 (`@/lib/mock/...`) 을 `@pullim-q/api-client` 호출로 치환:
  - **page wrapper (Server Component)** — `headers()` 로 `X-User-Id` 읽고 server-side prefetch 후 Container 에 props 로 주입. server-only fetch 책임은 Container 밖에 둠 (Phase 1~5 에서 정의한 Container = `'use client'` 레이어와 분리 유지)
  - **Container (Client Component, `'use client'`)** — Phase 1~5 정의 그대로: state + router + searchParams + 이벤트 핸들러. fetch 는 page wrapper 가 주입한 초기 데이터 사용 + client-side mutation 시 `@pullim-q/api-client` 호출
